...
// 우리가 콜백함수에 쓰는데 활용하는 데이터가 그닥 의미가 없는 것이 단점이다.
...
void CALLBACK RecvCallback(DWORD error, DWORD recvLen, LPWSAOVERLAPPED overlapped, DWORD flags)
{
	...
	// LPWSAOVERLAPPED에 쓰이는 방식은 포인터 방식이여서 구조체로 넘기고 첫 위치에 넣는다면, Session으로 다시 캐스팅이 가능하다.
	...
	while (true)
	{
		...
		while (true)
		{
			...
			if (::WSARecv(clientSocket, &wsaBuf, 1, &recvLen, &flags, &session.overlapped, RecvCallback) == SOCKET_ERROR)
			{
				...
				{
					// Pending
					// Alertable Wait 상태로 진입이 필요함
					// https://m.blog.naver.com/kbm0996/221124634245
					// APC Queue에 있는 것들이 모두 비워진 뒤에 탈출하게 된다.
					::SleepEx(INFINITE, TRUE);	// 해당 함수를 활용하거나
					//::WSAWaitForMultipleEvents(1, &wsaEvent, TRUE, WSA_INFINITE, TRUE); // 아래 함수로도 동작이 가능하다
				}
				else
				{
					// TODO : 문제 있는 상황
